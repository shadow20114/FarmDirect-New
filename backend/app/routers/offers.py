...
def place(produce_id:int=0,quantity:float=0,offered_price:float=0,current=Depends(get_current_user)):
    ...
    cur=conn.execute("INSERT INTO offers(produce_id,buyer_id,buyer_name,quantity,offered_price,status,created_at) VALUES(?,?,?,?,?,?,?)",(produce_id,uid,u["name"],quantity,offered_price,"PENDING",datetime.utcnow().isoformat()))

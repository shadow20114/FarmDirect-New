...
def place(produce_id:int=0,quantity:float=0,current=Depends(get_current_user)):
    ...
    cur=conn.execute("INSERT INTO orders(produce_id,buyer_id,farmer_id,quantity,unit_price,total_price,status,tracking_status,source_type,offer_id,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",(produce_id,uid,p["farmer_id"],quantity,p["expected_price"],total,"ACCEPTED","ACCEPTED","DIRECT",None,datetime.utcnow().isoformat()))

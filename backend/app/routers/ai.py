...
def price_predict(crop:str="Wheat",baseline:float=2400.0,demand:int=70,quantity:float=10):
    ...
    return {"crop":crop,"recommended_price":recommended,"mandi_baseline":baseline,"demand_index":demand,"demand_level":level,"retail_rate":retail,"price_change_percentage":change,"confidence":confidence,"factors":factors,"source":"farmdirect-ai-demo"}

@router.post("/demand")
def demand(crop_name:str,historical_demand:str,current_month:int,future_periods:int=1):
    ...
    if future_periods<1 or future_periods>12: raise HTTPException(400,"future_periods must be between 1 and 12")
    n=len(values);xs=list(range(1,n+1));mx=mean(xs);my=mean(values);den=sum((x-mx)**2 for x in xs);slope=sum((x-mx)*(y-my) for x,y in zip(xs,values))/den if den else 0;intercept=my-slope*mx
    preds=[max(0,round(intercept+slope*(n+i),2)) for i in range(1,future_periods+1)]
    confidence=round(min(95,max(58,65+min(25,len(values)*2))),1)
    return {"crop_name":crop_name,"current_month":current_month,"historical_demand":values,"predicted_demand":preds,"trend":"RISING" if slope>0.5 else "FALLING" if slope<-0.5 else "STABLE","confidence":confidence}
...
def selling_recommendation(crop_name:str,current_price:float,predicted_demand:float,average_market_price:float):
    ...
    score=min(100,score)
    if score>=75: action="SELL NOW"
    elif score>=55: action="SELL WITHIN 7-14 DAYS"
    else: action="WAIT AND MONITOR"
    reasons=[]
    reasons.append("demand is strong" if predicted_demand>=75 else "demand is moderate" if predicted_demand>=55 else "demand is soft")
    reasons.append("listing price is below the market reference" if current_price<average_market_price else "listing price is at or above the market reference")
...
def buyer_match(produce_id:int):
    ...
    if not p:
        conn.close();raise HTTPException(404,"Produce not found")
    ...
    by_id={int(x["buyer_id"]):x for x in offers}
    results=[]
    for b in buyers:
        score=55;reasons=[]
        offer=by_id.get(int(b["id"]))
        if offer:
...
def insights(produce_id:int):
    ...
    baseline=float(p["expected_price"]);quantity=float(p["quantity"]);idx,level=demand_score(quantity,baseline,p["crop_name"])
    prices=[float(x[0]) for x in history]
    market_avg=round(mean(prices),2) if prices else baseline
    predicted_demand=idx
    recommended=price_predict(p["crop_name"],baseline,idx,quantity)
    sell=selling_recommendation(p["crop_name"],baseline,predicted_demand,market_avg)
    demand_reason="Demand is high because the listing quantity is relatively limited and the crop signal is favorable." if idx>=75 else "Demand is moderate; monitor buyer activity and nearby market prices." if idx>=55 else "Demand is soft; avoid rushing into a low-price sale."
    return {"produce":dict(p),"price":recommended,"demand":{"index":idx,"level":level,"reason":demand_reason},"selling":sell}
